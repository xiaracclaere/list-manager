...
import os, json, time, random, configparser
...
class SetupAll:
    ...
    def setup_showing_commands(self):
        ...
        for command in main_choices_all:
            if self.options["ShowCommandsNormalMode"][command] == "true":
                main_choices_showing.append(command)
        for command in debug_choices_all:
            if self.options["ShowCommandsDebugMode"][command] == "true":
                debug_choices_showing.append(command)
    
    def __call__(self):
        self.load_config()
        self.setup_showing_commands()
...
class functions:
    ...
    def load(lch, name="lmdata.json", list_number=1, maxlnrange=counter):
