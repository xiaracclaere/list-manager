    ...
    try:
        def wrapper(*args, **kwargs):
            try:
                ...
                functions.ErrorLog(f"Error: {str(e)}")
        return wrapper
    except:
        TooManyErrors()
...
class functions:
    ...
    def load(lch):
        ...
        try:
            with open(f"{lch}.pkl", "rb") as file:
                lst = pickle.load(file)
        except FileNotFoundError as e:
            functions.ErrorLog(f"FileNotFoundError: {str(e)}")
            LoadingListError()
            lst = []
        except EOFError as e:
            functions.ErrorLog(f"EOFError: {str(e)}")
            LoadingListError()
            lst = []
        except Exception as e:
            functions.ErrorLog(f"Unknown error: {str(e)}")
            LoadingListError()
            lst = []
...
- 
Happy News:
The code is changed by {816-lns} lines!
...
def main_mode():
    global mode
    action = input(f"| Add | Delete | List | Clean | Sort | New | Lists | Export | Help | History | Exit | \n").strip().lower()
...
def debug_mode():
    ...
    global using
    global timer
    action = input(f"| Add | Delete | List | Clean | Clear | Sort | Export | Timer | Performance | Use | Reset | Code | Help | Mode | History | Exit | \n").strip().lower()
...
def main():
    ...
    functions.save(lch="list", list_name=f"list{counter}")
