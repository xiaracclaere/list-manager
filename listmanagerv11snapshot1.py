...
def load(list_name="list"):
    ...
    try:
        ...
        ProgramErrorLog(f"FileNotFoundError: {str(e)}")
        LoadingListError()
        lst = []
    except EOFError as e:
        ProgramErrorLog(f"EOFError: {str(e)}")
        LoadingListError()
        lst = []
    except Exception as e:
        ProgramErrorLog(f"Unknown error: {str(e)}")
        LoadingListError()
        lst = []
...
- Some experimental commands are hidden. 
- Now program is not experimental, it's user friendly. 
Happy News:
The code increased from 356 to {lns} lines!
...
def main():
    ...
    print('Welcome to the List Manager! You can write "help" to get help. Version = 11s1 ')
...
"""
List Manager Version 11 Snapshot 1(Complete)
